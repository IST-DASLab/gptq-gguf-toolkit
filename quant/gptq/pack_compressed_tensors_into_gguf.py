            ...
            for new_name, curr in self.modify_tensors(data_torch, name, bid):
                if has_packed_quant:
                    ...
                    try:
                        ...
                        data = gguf.quants.quantize(curr.numpy(), data_qtype)

                    shape = gguf.quant_shape_from_byte_shape(data.shape, data_qtype) if getattr(data, "dtype", None) is np.uint8 else data.shape
                    ...
                    if isinstance(data_qtype, bool):
                        ft = self.ftype
                        if ft == gguf.LlamaFileType.ALL_F32:
                            data_qtype = gguf.GGMLQuantizationType.F32
                        elif ft == gguf.LlamaFileType.MOSTLY_F16:
                            data_qtype = gguf.GGMLQuantizationType.F16
                        elif ft == gguf.LlamaFileType.MOSTLY_BF16:
                            data_qtype = gguf.GGMLQuantizationType.BF16
                        elif ft == gguf.LlamaFileType.MOSTLY_Q8_0:
                            data_qtype = gguf.GGMLQuantizationType.Q8_0
                        elif ft == gguf.LlamaFileType.MOSTLY_TQ1_0:
                            data_qtype = gguf.GGMLQuantizationType.TQ1_0
                        elif ft == gguf.LlamaFileType.MOSTLY_TQ2_0:
                            data_qtype = gguf.GGMLQuantizationType.TQ2_0
                        else:
                            raise ValueError(f"Unknown file type: {ft.name}")

                    try:
                        data = gguf.quants.quantize(data_np, data_qtype)
                    except gguf.QuantError as e:
                        logger.warning("%s, %s", e, "falling back to F16")
                        data_qtype = gguf.GGMLQuantizationType.F16
                        data = gguf.quants.quantize(data_np, data_qtype)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dir_model_quant", type=Path,
        help="directory containing quantized model file",
        nargs="?",
    )
    parser.add_argument(
